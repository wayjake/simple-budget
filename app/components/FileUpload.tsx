...
const FileUpload = ({ className, onUpload }: Props) => {
    ...
    const handleFileUpload = (e) => {
        ...
        reader.onload = function (event) {
            ...
            const transactions = lines.slice(0).map(line => {
                ...
                return { date: date?.slice(1, -1), description: description?.slice(1, -1), id: generateSmallRandomId(), amount: Big(amount?.slice(1, -1)) };
            });
    ...
    return (
        <div className={className}>
            <input type="file" accept=".csv" onChange={handleFileUpload} className="bg-card-light text-text-dark dark:bg-card dark:text-text" />
        </div>
    );
};
